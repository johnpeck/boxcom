...
** TODO Work through chapter 4.5 of Pratt
...
** TODO Make a variable current_cal scoped global to current module
...
** TODO Write function to load current calibration factors (in cal module)
...
** TODO Write remote command to write current slope
   - Call this $curslp
** TODO Write function to write current offset (in cal module)
   - Offset will be a 16-bit signed integer.  The function will have
     to break this up according to the cal factor structure.
** TODO Write remote command to write current offset
   - Call this $curoff
** TODO Write function to write current offset
** TODO Restructure how remote commands work
*** DONE Add a command argument structure
    - Remote commands will take pointers to this structure instead of
*** DONE Rename "hex" type to "hex16"
    - These will be unsigned 16-bit integers that come in with hex formatting
*** TODO Fix remote commands to use the new structure
    - As commands are fixed, add them to the user command list in the
      howto.  Use logreg as a template.
** TODO Add command handler for 16-bit integers
